...
class LLMClient:
    def __init__(self, provider: str, model_name: str, compute_type: str, api_key: str = None):
        ...
        logger.info(f"LLMClient initialized for {provider}")
        ...
        if self.provider == "Local (Ollama)":
            try:
                ...
                if res.status_code == 200:
                    ...
                    # Direct match or partial match (e.g. 'llama3.1' in 'llama3.1:latest')
            ...
            try:
                if "gemini" in self.model_name.lower():
                    ...
                    res = requests.get(url, timeout=10) # Increased timeout
                ...
                # Default success for others if keys are present (reduces cost/probing time)
