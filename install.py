...
APP_PACKAGES = [
    ...
    "pydantic",
    "httpx",
    # Security packages — required for rate limiting and vault encryption
    "slowapi",
    "cryptography",
]
...
def ensure_env_file():
    ...
    if env_path.exists():
        ...
        # Rotate the key if it still contains the well-known placeholder values
        content = env_path.read_text(encoding="utf-8")
        if "replace_this_with_a_secure_key" in content or "dev-key-12345" in content:
            import secrets
            new_key = secrets.token_hex(32)
            content = re.sub(
                r"^(INTERNAL_API_KEY\s*=\s*).*$",
                rf"\g<1>{new_key}",
                content,
                flags=re.MULTILINE
            )
            env_path.write_text(content, encoding="utf-8")
            ok(f".env already existed — rotated placeholder INTERNAL_API_KEY to a secure random key")
        return
    ...
    if example_path.exists():
        ...
        # Replace placeholder with a freshly generated cryptographic key
        content = re.sub(
            r"^(INTERNAL_API_KEY\s*=\s*).*$",
            rf"\g<1>{secure_key}",
            content,
        ...
        env_path.write_text(content, encoding="utf-8")
        ok(".env created from .env.example with a secure INTERNAL_API_KEY")
        info("Open .env to fill in your AI provider API keys before first use.")
    else:
        warn(".env.example not found — creating a minimal .env")
        env_path.write_text(
            f"# AI Interview Coach — generated by install.py\n"
            f"INTERNAL_API_KEY={secure_key}\n"
            f"API_URL=http://127.0.0.1:8000\n"
            f"OLLAMA_HOST=http://127.0.0.1:11434\n"
            f"OPENAI_API_KEY=\n"
            f"ANTHROPIC_API_KEY=\n"
            f"GOOGLE_API_KEY=\n",
            encoding="utf-8"
